...
def parse_usa_state(country_str):
    """
    ...
    # "ST, CITY" or "CITY, ST" – check the last token first
    if ',' in remainder:
        parts = [p.strip() for p in remainder.split(',')]
        # Try each part as a state abbreviation or name
        for part in reversed(parts):          # last part is usually state
            part = part.strip()
            if part in STATE_TO_HHS:
                return part if len(part) == 2 else _abbrev_from_name(part)
        # Fall through to city lookup using the first part
        for part in parts:
            if part in CITY_TO_STATE:
                return CITY_TO_STATE[part]
        return None

    # bare two-letter abbreviation
    if re.fullmatch(r'[A-Z]{2}', remainder):
        return remainder if remainder in STATE_TO_HHS else None

    # full state name
    if remainder in STATE_TO_HHS:
        return _abbrev_from_name(remainder)

    # known city
    if remainder in CITY_TO_STATE:
        return CITY_TO_STATE[remainder]
...
for _name in STATE_TO_HHS:
    if len(_name) > 2 and _name not in _NAME_TO_ABBREV:
        _initials = ''.join(w[0] for w in _name.split())
        if _initials in STATE_TO_HHS:
            _NAME_TO_ABBREV[_name] = _initials
...
for meta_file in glob.glob('./tables/*metadata.csv'):
    ...
    has_month['state'] = has_month['country'].apply(parse_usa_state)
